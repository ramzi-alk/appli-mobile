...
if (isset($_SESSION['sess_user_id']) && isset($_SESSION['sess_user_name'])) { ?>
    ...
    <head>
        ...
        <title>Modification de la vitre</title>
    </head>
                ...
                <div class="formulaire">
                    <?php if (isset($_GET['id']) == true && $_GET['id'] > 0 && isset($_GET['idVit']) == true && $_GET['idVit'] > 0) {
                        ...
                        try {
                            ...
                            $lesEnregs = $bdd->query("select id_modele as 'idMod', id_vitreArriere as 'id_vit', nombreStock as 'nbStock',prix_achat from modele_vitrearriere where id_modele = $id and id_vitreArriere = $idVit");
                        } catch (PDOException $e) {
                            die("Err BDselect : erreur de lecture table vitrearriere dans index.php<br>Message d'erreur :" . $e->getMessage());
                        }
                        ...
                        if (isset($_POST['envoyer']) == true) {
                            ...
                            try {
                                ...
                                $req = $bdd->prepare("update modele_vitrearriere set nombreStock = :par_nbStock, prix_achat = :par_prix where id_modele = :par_id and id_vitreArriere = :par_idVit  ");
                   ...
                   window.location.replace('/page/stock_vitre.php');
                   </script>";
                                }
                            } catch (PDOException $e) {
                                die("Err BDselect : erreur de lecture table vitrearriere dans index.php<br>Message d'erreur :" . $e->getMessage());
