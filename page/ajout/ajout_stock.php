...
<?php include $_SERVER['DOCUMENT_ROOT'] . "/include/btn.html"; ?>
    ...
    <?php 
        ...
        if(isset($_POST['envoyer']) == true){
            ...
            $existe = $bdd->query("select count(*) from modele_vitrearriere where id_modele = $nom and id_vitreArriere = $couleur ");
            $siExiste = $existe->fetchColumn();
            if($siExiste == 0){
                try{
                
                    $req = $bdd->prepare("INSERT INTO `modele_vitrearriere` (`id_modele`, `id_vitreArriere`, `prix_achat`,`nombreStock`) VALUES (:par_nom, :par_couleur,:par_prix ,:par_nbStock)");
